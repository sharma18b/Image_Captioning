...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        for t in range(max(decode_lengths)):
            ...
            input_tensor = torch.cat([embeddings[:batch_size_t, t, :], attention_weighted_encoding], dim=1)
            hs_forward, cs_forward = self.lstm(input_tensor, (hs_forward, cs_forward))
        ...
        for t in range(max(decode_lengths)):
            ...
            attention_weighted_encoding = gate * attention_weighted_encoding
            input_tensor = torch.cat([embeddings[:batch_size_t, t, :], attention_weighted_encoding], dim=1)
            hs_backward, cs_backward = self.lstm_cell_backward(input_tensor, (hs_backward, cs_backward))
        ...
        for fwd, bwd in zip(forward, backward):
            ...
            preds = self.fc(self.dropout(hs_lstm))  # [batch_size_t, vocab_size]
        ...
        for t in range(max(decode_lengths)):
            ...
            input_tensor_fwd = torch.cat([embeddings[:batch_size_t, t, :], attention_weighted_encoding], dim=1)
            h, c = self.lstm(
                input_tensor_fwd,
