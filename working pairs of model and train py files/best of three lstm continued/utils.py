...
def save_checkpoint(data_name, epoch, epochs_since_improvement, encoder, decodera,decoderb,decoderc,decoderf, encoder_optimizer, decodera_optimizer,decoderb_optimizer,decoderc_optimizer,decoderf_optimizer,
                    metrics, is_besta,is_bestb,is_bestc,is_bestf, final_args):
    """
    ...
    state = {'epoch': epoch,
             ...
             'decodera_optimizer': decodera_optimizer,
             'decoderb_optimizer': decoderb_optimizer,
             'decoderc_optimizer': decoderc_optimizer,
             'decoderf_optimizer': decoderf_optimizer,
             'final_args': final_args}
    filename = 'checkpoint_' + data_name + '.pth.tar'
    torch.save(state, filename)
    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_besta:
        torch.save(state, 'BEST_A_' + filename)
    if is_bestb:
        torch.save(state, 'BEST_B_' + filename)
    if is_bestc:
        torch.save(state, 'BEST_C_' + filename)
    if is_bestf:
        torch.save(state, 'BEST_F_' + filename)
