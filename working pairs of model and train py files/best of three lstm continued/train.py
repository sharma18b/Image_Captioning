...
def train(args, train_loader, encoder, decodera, decoderb, decoderc, decoderf, criterion,mse, encoder_optimizer, decodera_optimizer, decoderb_optimizer, decoderc_optimizer,decoderf_optimizer, epoch):
    """
    ...
    for i, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        lossab = mse(scoresa, scoresb) #criterion(scoresa, scoresb)
        lossbc = mse(scoresb, scoresc) #criterion(scoresb, scoresc)
        lossca = mse(scoresc, scoresa) #criterion(scoresc, scoresa)
        #print("A B C crossed")
        ...
        if(abs(lossa) <= abs(lossb) and abs(lossa) <= abs(lossc)):
          scoresf, caps_sortedf, decode_lengthsf, alphasf, sort_indf = decoderf(imgs, caps, caplens,scoresa)
        elif(abs(lossb) <= abs(lossa) and abs(lossb) <= abs(lossc)):
          scoresf, caps_sortedf, decode_lengthsf, alphasf, sort_indf = decoderf(imgs1, caps, caplens,scoresb)
        else:
          scoresf, caps_sortedf, decode_lengthsf, alphasf, sort_indf = decoderf(imgs2, caps, caplens,scoresc)
        #print("decoderf cross ho gya")
        targetsf = caps_sortedf[:, 1:]
        scoresf = pack_padded_sequence(scoresf, decode_lengthsf, batch_first=True).data
        targetsf = pack_padded_sequence(targetsf, decode_lengthsf, batch_first=True).data
        # print(targetsf.shape)
        # print(targetsc.shape)
        # print(scoresf.shape)
        lossf = criterion(scoresf, targetsf)
        lossf += args.alpha_c * ((1. - alphasf.sum(dim=1)) ** 2).mean()
        #print("crossed ab age dekh")
        ...
        ## for f
        lossesf.update(lossf.item(), sum(decode_lengthsf))
        top5accsf.update(top5f, sum(decode_lengthsf))
        #print("crossed ab age dekh 5")
        # ## for a
        ...
        if i % args.print_freq == 0:
            print("Epoch: {}/{} step: {}/{} \n Loss-A: {} Loss-B: {} Loss-C: {} Loss-AB: {} Loss-BC: {} Loss-CA: {} \n AVG_Loss_A: {} AVG_Loss_B: {} AVG_Loss_C: {} \n Top-5 Accuracy_A: {} Top-5 Accuracy_B: {} Top-5 Accuracy_C: {} \n Loss-Final: {} AVG-Loss F: {} Top-5 Accuracy_F : {} Batch_time: {}s".format(epoch+1, args.epochs, i+1, len(train_loader), lossesa.val, lossesb.val, lossesc.val, lossab, lossbc, lossca, lossesa.avg, lossesb.avg, lossesc.avg, top5accsa.val, top5accsb.val, top5accsc.val,lossesf.val, lossesf.avg, top5accsf.val, batch_time.val))


def validate(args, val_loader, encoder, decodera,decoderb,decoderc,decoderf, criterion, mse):
    """
    ...
    decoderf.eval()  # eval mode (no dropout or batchnorm)
    ...
    lossesb = AverageMeter()  # loss (per word decoded)
    lossesc = AverageMeter()  # loss (per word decoded)
    lossesf = AverageMeter()  # loss (per word decoded)
    top5accsa = AverageMeter()  # top5 accuracy
    top5accsb = AverageMeter()  # top5 accuracy
    top5accsc = AverageMeter()  # top5 accuracy
    top5accsf = AverageMeter()  # top5 accuracy
    ...
    referencesf = list()  # references (true captions) for calculating BLEU-4 score
    hypothesesf = list()  # hypotheses (predictions)
    ...
    with torch.no_grad():
        ...
        for i, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            top5accsc.update(top5c, sum(decode_lengthsc))  
            lossesf.update(lossf.item(), sum(decode_lengthsf))
            top5accsf.update(top5f, sum(decode_lengthsc))            
            # losses.update(loss.item(), sum(decode_lengths))
            ...
            allcapsf = allcaps[sort_indf]  # because images were sorted in the decoder
            ...
            for j in range(allcapsf.shape[0]):
                img_caps = allcapsc[j].tolist()
                img_captions = list(
                    map(lambda c: [w for w in c if w not in {word_map['<start>'], word_map['<pad>']}],
                        img_caps))  # remove <start> and pads
                referencesf.append(img_captions)
            ...
            # Hypotheses For Final
            _, preds = torch.max(scoresf_copy, dim=2)
            preds = preds.tolist()
            temp_preds = list()
            for j, p in enumerate(preds):
                temp_preds.append(preds[j][:decode_lengthsf[j]])  # remove pads
            preds = temp_preds
            hypothesesf.extend(preds)

            assert len(referencesa) == len(hypothesesa)
            assert len(referencesb) == len(hypothesesb)
            assert len(referencesc) == len(hypothesesc)
            assert len(referencesf) == len(hypothesesf)
    ...
    metricsf = get_eval_score(referencesf, hypothesesf)
    ...
    print("EVA LOSS: {} TOP-5 Accuracy {} BLEU-1 {} BLEU2 {} BLEU3 {} BLEU-4 {} ROUGE_L {} CIDEr {}".format
          (lossesc.avg, top5accsc.avg,  metricsf["Bleu_1"],  metricsf["Bleu_2"],  metricsf["Bleu_3"],  metricsf["Bleu_4"],
           metricsf["ROUGE_L"], metricsf["CIDEr"]))

    return metricsa,metricsb,metricsc,metricsf
...
if __name__ == '__main__':
    ...
    parser.add_argument('--decoder_lr', type=float, default=1e-3, help='learning rate for decoder.')
    ...
    parser.add_argument('--checkpoint', default='./checkpoint_flickr30k_5_cap_per_img_5_min_word_freq.pth.tar', help='path to checkpoint, None if none.')
    ...
    start_epoch = 0
    best_bleu4 = 0.  # BLEU-1 score right now
    best_bleu1a = 0.  # BLEU-1 score right now
    best_bleu1b = 0.  # BLEU-1 score right now
    best_bleu1c = 0.  # BLEU-1 score right now
    best_bleu1f = 0.  # BLEU-1 score right now
    ...
    if args.checkpoint is None:
        ...
        if args.decoder_mode == "lstm":
            ...
            decoderc = DecoderWithAttention(attention_dim=args.attention_dim,
                                           ...
                                           vocab_size=len(word_map),
                                           dropout=args.dropout)
            decoderf = DecoderWithAttention2(attention_dim=args.attention_dim,
                                           embed_dim=args.emb_dim,
                                           decoder_dim=args.decoder_dim,
                                           vocab_size=len(word_map),
                                           dropout=args.dropout)
        ...
        decoderc_optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, decoderc.parameters()),
                                             lr=args.decoder_lr)
        decoderf_optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, decoderf.parameters()),
                                             lr=args.decoder_lr)
        ...
        if args.embedding_path is not None:
            ...
            decoderf.load_pretrained_embeddings(word_embeds)
            decoderf.fine_tune_embeddings(args.fine_tune_embedding)
        ...
        decodera = checkpoint['decodera']
        decodera_optimizer = checkpoint['decodera_optimizer']
        decodera.fine_tune_embeddings(args.fine_tune_embedding)
        decoderb = checkpoint['decoderb']
        decoderb_optimizer = checkpoint['decoderb_optimizer']
        decoderb.fine_tune_embeddings(args.fine_tune_embedding)
        decoderc = checkpoint['decoderc']
        decoderc_optimizer = checkpoint['decoderc_optimizer']
        decoderc.fine_tune_embeddings(args.fine_tune_embedding)
        decoderf = checkpoint['decoderf']
        decoderf_optimizer = checkpoint['decoderf_optimizer']
        decoderf.fine_tune_embeddings(args.fine_tune_embedding)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train(args, train_loader=train_loader, encoder=encoder, decodera=decodera,decoderb=decoderb,decoderc=decoderc,decoderf=decoderf, 
            criterion=criterion,mse = mse, encoder_optimizer=encoder_optimizer, decodera_optimizer=decodera_optimizer, 
            decoderb_optimizer=decoderb_optimizer, decoderc_optimizer=decoderc_optimizer,decoderf_optimizer=decoderf_optimizer, epoch=epoch)

        # One epoch's validation
        metricsa, metricsb, metricsc, metricsf = validate(args, val_loader=val_loader, encoder=encoder, decodera=decodera,decoderb=decoderb,decoderc=decoderc,decoderf=decoderf,
            criterion=criterion, mse = mse)
        recent_bleu1a = metricsa["Bleu_1"]
        recent_bleu1b = metricsb["Bleu_1"]
        recent_bleu1c = metricsc["Bleu_1"]
        recent_bleu1f = metricsf["Bleu_1"]

        # Check if there was an improvement
        is_besta = recent_bleu1a > best_bleu1a
        is_bestb = recent_bleu1b > best_bleu1b
        is_bestc = recent_bleu1c > best_bleu1c
        is_bestf = recent_bleu1f > best_bleu1f
        best_bleu1a = max(recent_bleu1a, best_bleu1a)
        best_bleu1b = max(recent_bleu1b, best_bleu1b)
        best_bleu1c = max(recent_bleu1c, best_bleu1c)
        best_bleu1f = max(recent_bleu1f, best_bleu1f)
        if not is_bestf:
        ...
        # Save checkpoint
        save_checkpoint(args.data_name, epoch, epochs_since_improvement, encoder, decodera,decoderb,decoderc,decoderf, encoder_optimizer,
                        decodera_optimizer,decoderb_optimizer,decoderc_optimizer,decoderf_optimizer, metricsa, is_besta,is_bestb,is_bestc,is_bestf, final_args)
