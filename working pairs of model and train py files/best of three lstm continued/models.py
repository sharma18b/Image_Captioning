...
class CNN_Encoder(nn.Module):
    ...
    def __init__(self, encoded_image_size=14, attention_method="ByPixel"):
        ...
        modules_1 = list(resnet.children())[:-2]
        modules_2 = list(resnet.children())[:-3]
        ...
        self.adaptive_pool7 = nn.AdaptiveAvgPool2d((7,7))
        self.adaptive_pool28 = nn.AdaptiveAvgPool2d((28,28))
        self.channelFrom_1024_To_2048 = nn.Conv2d(1024,2048,(1))
        self.SimpleCon2d_channelFrom_2048_To_2048 = nn.Conv2d(2048,2048,(1))
    ...
    def forward(self, images):
        ...
        out_1 = self.resnet_1(images)  # (batch_size, 2048, image_size/32, image_size/32)
        out_2 = self.resnet_2(images)  # (batch_size, 2048, image_size/32, image_size/32)
        if self.attention_method == "ByChannel":  # [batch_size, 2048, 8, 8] -> # [batch_size, 512, 8, 8]
            out = self.relu(self.bn1(self.cnn1(out)))
            out_1 = self.relu(self.bn1(self.cnn1(out_1)))
            out_2 = self.relu(self.bn1(self.cnn1(out_2)))
        # print(out.shape)
        # print(out_1.shape)
        # print(out_2.shape)
        out = self.adaptive_pool(out)  # [batch_size, 2048/512, 8, 8] -> [batch_size, 2048, 14, 14] 32, 2048, 14, 14
        out_1 = self.adaptive_pool7(out_1)  # [batch_size, 2048/512, 8, 8] -> [batch_size, 1024, 7, 7] 32, 2048, 7, 7
        out_2 = self.adaptive_pool28(out_2)  # [batch_size, 2048/512, 8, 8] -> [batch_size, 2048, 14, 14] 32, 1024, 28, 28
        # out_1_upsampled = self.upsample2(out_1) ## 32,2048,14,14
        out = self.relu(self.SimpleCon2d_channelFrom_2048_To_2048( self.upsample2(out_1) ) + out)
        out_2 = self.relu(self.channelFrom_1024_To_2048( out_2 ) + self.upsample2(out))
        #out_2 = self.relu(self.channelFrom_1024_To_2048( self.down2(out_2) ) + out)
        ##out_2 = self.channelFrom_1024_To_2048(out_2)
        # # print(out_1_upsampled.shape)
        # out_1_mid = self.SimpleCon2d_channelFrom_2048_To_2048(out_1_upsampled)  ## 32,2048,14,14
        # # print(out_1_mid.shape)
        # # print(out.shape)
        # out = self.relu(out + out_1_mid)  # 32,2048,14,14
        # out_upsampled = self.upsample2(out) ## 32,2048,28,28
        # out_mid = self.SimpleCon2d_channelFrom_2048_To_2048(out_upsampled)
        # # print(out_mid.shape)
        # # print(out_2.shape)
        #out_2 = self.channelFrom_1024_To_2048(out_2)
        # out_2 = self.relu(out_2 + out_mid) #32 ,2048 , 28,28
        # # print(out_1.shape)
        # # print(out.shape)
        # # print(out_2.shape)
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths):
        ...
        for t in range(max(decode_lengths)):
            ...
            # print("inside loop " + str(t))
            # print( " embeddings[:batch_size_t, t, :] " )
...
class DecoderWithAttention2(nn.Module):
    ...
    def __init__(self, attention_dim, embed_dim, decoder_dim, vocab_size, encoder_dim=2048, dropout=0.5):
        ...
        self.embedding = nn.Embedding(vocab_size, embed_dim, padding_idx=0)  # embedding layer
        self.dropout = nn.Dropout(p=self.dropout)
        self.lstm = nn.LSTMCell(embed_dim + encoder_dim + vocab_size, decoder_dim, bias=True)  # decoding LSTMCell
        self.init_h = nn.Linear(encoder_dim, decoder_dim)  # linear layer to find initial hidden state of LSTMCell
        self.init_c = nn.Linear(encoder_dim, decoder_dim)  # linear layer to find initial cell state of LSTMCell
        self.f_beta = nn.Linear(decoder_dim, encoder_dim)  # linear layer to create a sigmoid-activated gate
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths, preds_previous):
        """
        Forward propagation.
        ...
        for t in range(max(decode_lengths)):
            batch_size_t = sum([l > t for l in decode_lengths])
            # alpha: [batch_size_t, 196]
            # attention_weighted_encoding: [batch_size_t, 2048] ,preds_previous[:batch_size_t, t, :]
            # print(" preds_previous[:batch_size_t, t, :] ")
            # print(preds_previous[:batch_size_t, :].shape)
            # print(" encoder_out[:batch_size_t].shape ")
            # print(encoder_out[:batch_size_t].shape)
            # print(" h[:batch_size_t].shape ")
            # print(h[:batch_size_t].shape)
            #tes = torch.cat([ h[:batch_size_t], preds_previous[:batch_size_t, :]], dim=1)
            #print(tes.shape)
            attention_weighted_encoding, alpha = self.attention(encoder_out[:batch_size_t], h[:batch_size_t])
            ...
            attention_weighted_encoding = gate * attention_weighted_encoding
            # torch.cat([batch_size_t, 500], [batch_size_t, 2048], dim=1) = [batch_size_t, 2548] -> [batch_size_t, 512]
            tes1 = torch.cat([embeddings[:batch_size_t, t, :], attention_weighted_encoding], dim=1)
            tes2 = torch.cat([tes1 , preds_previous[:batch_size_t, :]], dim=1)
            # print(tes2.shape)
            # print(" crossed awe formation ")
            h, c = self.lstm(
                tes2,
                (h[:batch_size_t], c[:batch_size_t]))
            #print(" crossed awe formation 1 ")
            preds = self.fc(self.dropout(h))  # [batch_size_t, vocab_size]
            predictions[:batch_size_t, t, :] = preds
            alphas[:batch_size_t, t, :] = alpha
